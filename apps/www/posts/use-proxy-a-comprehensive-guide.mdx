...
```bash
npx @usehooks/latest add use-proxy
```
...
```tsx
import { useProxy } from "@usehooks/use-proxy";
...
I need to examine the useProxy hook implementation to create a detailed post explaining how to use it.

packages/hooks/src/use-proxy/index.ts :( 1 - 86 )

Now I need to examine the documentation to understand the examples and use cases for the useProxy hook.

packages/hooks/src/use-proxy/doc.json :( 1 - 87 )
...
```
npm install @usehooks/use-proxy
```

Or if you're using yarn:

```
yarn add @usehooks/use-proxy
```
...
  return (
    <div>
      <p>Name: {proxy.name}</p> {/* Will log "Accessing property: 
      name" */}
      <p>Age: {proxy.age}</p> {/* Will log "Accessing property: 
      age" */}
    </div>
  );
}
